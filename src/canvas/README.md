...
- `toWebp(maxEdge, quality=0.85)`: 紙色の上に完了ストロークだけ（グリッド・重ね・反転・シルエットは含めない）。解像度は CSS px × min(DPR, maxEdge / 長辺)。`toBlob('image/webp')` が null なら PNG。attach 前でも document があればストロークの範囲から大きさを決めて書き出す。
...
10. 書き出し: `toWebp(1024)` の Blob を `URL.createObjectURL` で開き、紙色＋線のみで、グリッド・重ねが入っていないこと。

## 既知の制限

- 描画部分（engine.ts の Canvas 操作・Pointer Events）は自動テストがない（jsdom/Canvas が無いため）。Playwright での実操作テストは UI 側で行う想定。
