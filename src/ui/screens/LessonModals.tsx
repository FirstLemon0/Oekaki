...
function Dialog({ labelId, class: cls, children }: { labelId: string; class: string; children: ComponentChildren }) {
...
export function LessonDoneModal({ summary, node, onMore }: { summary: LessonSummary; node: PathNode; onMore: () => void }) {
  const goHome = () => navigate(`#/?justDone=${encodeURIComponent(summary.lessonId)}`);
  const c = summary.counter;
  return (
    <Dialog labelId="ls-done-title" class="ls-done">
      ...
      <p class="ls-done__meta">
        {node.lesson.title} · <span class="num">{summary.elapsedMin}</span>分
      </p>
      <div class="ls-stats">
        <div class="ls-stat">
        ...
        <div class={c ? 'ls-stat is-up' : 'ls-stat'}>
          <span class="ls-stat__label">{c ? COUNTER_LABEL[c.kind].name : '累計'}</span>
          {c ? (
            <>
...
export function StageDoneModal({ summary, node }: { summary: LessonSummary; node: PathNode }) {
  ...
  const goHome = () => navigate(`#/?justDone=${encodeURIComponent(summary.lessonId)}`);
  return (
    <Dialog labelId="ls-stage-title" class="ls-stage">
