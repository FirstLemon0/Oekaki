...
- **フルツール（お絵描き v2、2026-09-25）**: 自由お絵描き（`#/free`）と、採点しない描くステップ（copy／construct／mosha／gesture／free／critique の canvas モード）は `CanvasScreen full`。採点する画面（ドリル・なぞり・校正・復習）は従来の簡易ツール（ペン／消しゴム／補助線／Undo／お手本／グリッド／反転／シルエット／再生／紙を替える）のまま。どちらもビュー（2 本指のズーム・パン・回転はエンジン、右上のズームピル）は使える
  ...
  - **塗りつぶしパネル**: 許容値スライダー（表示 0〜100、エンジンは 0〜255。既定 32＝表示 13）、境界のセグメント（このレイヤー／すべて）、注記「色はペンの色を使います」。端末内の好み `seichotsu.fillStyle`。塗りつぶしアイコンの下にもペンの現在色の丸
  - **図形パネル**: 直線／四角／楕円（プリセットと同じ 64px の選択肢）。選んだら閉じる。最後に選んだ図形を `seichotsu.shapeTool` に覚え、ツールバーのボタンはその図形のアイコンと名前「図形（楕円）」
  - **スポイト**: 紙をタップ → エンジンがペンの色にする（'toolchange'）。UI は色を直近に記憶し、スポイトの前のツール（ペンか塗りつぶし）へ戻る
  ...
  - **レイヤーパネル**: ツールバーの「レイヤー」で開閉。ツールバーの反対側の端のドロワー（幅 300、top 72 / bottom 88 で右上のピルと完了ボタンを避ける、glass・radius 14）。縦向きは下からのシート（高さ min(52dvh, 600)、surface、上角 20）。ヘッダ「レイヤー」15 700 ＋ mono の枚数 ＋ 閉じる 48。行 56px（上が手前）: サムネイル 40（市松の地、`getLayerThumbnail`、描き終えてから 300ms 後に更新）、名前 15（アクティブ行でタップすると入力欄に替わり改名）、目 48（非表示は ink-3 ＋ 行を .45）、鍵 48（ロック中は danger-text）。アクティブ行は accent-soft。行の長押し 400ms でつかんで上下へ並べ替え（つかんだ行は surface ＋ shadow-2、他の行は 160ms で詰める）。下部はアクティブ行の「不透明度」スライダー（ドラッグ中もその場で反映、続けての変更はエンジンが 1 手にまとめる。右に mono %）、「合成」セグメント（通常／乗算／スクリーン）、「手前へ／奥へ」48 ＋ 注記「長押しでつかんで並べ替え」12 ink-2。フッタ 4 等分の 56px ボタン（アイコン 22 ＋ 11px の名前）「追加」「複製」「下と結合」「削除」（削除は danger-text、確認モーダル「『名前』を消しますか」→ 副「やめる」・危険「消す」）。一番下のレイヤーは結合できない、1 枚だけなら削除できない（無効表示）
  - **ロック中の案内**: アクティブなレイヤーがロック中で描くツールを選んでいるとき、下中央に 40px の glass ピル（鍵 18 ＋ 13 ink-2「このレイヤーはロック中です。描くときはレイヤーの鍵を外します」）。静的な表示で、描いている間に出たり消えたりしない
  ...
  - **ズームピル**: 右上（カウンターの右）に 40px の glass、mono 15 500「100%」。タップで「全体を表示」（`fitView`）。回転していれば左に「回転を戻す」（アイコン 20 ＋ mono の角度）。すべてのキャンバス画面（ジェスチャーを除く簡易ツールも）に出す
  - **「終わる」シート（自由お絵描き）**: 採点シートと同じ位置の surface シート（高さは内容まで）。「終わりますか」22 700、説明 15 ink-2、フッタ右寄せに 副「透過 PNG を書き出す」（download アイコン）・文字「描き続ける」・主「保存して終わる」。書き出したら「透過 PNG を書き出しました。」14 accent-text。ファイル名は `seichotsu-free-YYYYMMDD-HHMM.png`
  - **保存（レイヤー等を使った絵）**: レイヤーが 2 枚以上、または stroke 以外の操作（塗り・変形・削除・レイヤー操作）があれば `meta.doc = engine.getDocument()`（strokes / strokeStyles も互換のため持つ）。それ以外は従来どおり。下書き（sessionStorage）も同じ条件で文書を残し、`loadDocument` で戻す。画面の回転・大きさの違う下書きは、文書の座標を「中心合わせ・短辺の比」で写してから `loadDocument`（旧 API の loadHistory はレイヤーを消すため使わない）。ギャラリーの再生は `doc` があれば `loadDocument` → `replay`、詳細の副「PNG で書き出す」は紙色つき PNG（長辺 2048）
